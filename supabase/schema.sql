-- ══════════════════════════════════════════════════════════════
-- QuizPlatform – Full Supabase Schema
...
-- ══════════════════════════════════════════════════════════════
-- QUESTIONS  (quiz-specific when quiz_id is set; bank-only when null)
-- ══════════════════════════════════════════════════════════════
create table questions (
  ...
  -- mcq_single/mcq_multi: { options: [{id,text}], correct: string|string[] }
  -- true_false:  { correct: boolean }
  -- fill_blank:  { correct: string, caseSensitive: boolean }
...
-- 2. Auto-create profile + profile_stats on Supabase Auth signup
...
-- 6. Auto-issue certificate when attempt passes and certificate_enabled = true
...
-- Helper: is admin/super_admin
create or replace function is_admin()
returns boolean language sql security definer stable as $$
  select coalesce(current_user_role() in ('admin','super_admin'), false)
$$;

-- Helper: is instructor or above
create or replace function is_instructor()
returns boolean language sql security definer stable as $$
  select coalesce(current_user_role() in ('admin','super_admin','instructor'), false)
$$;
...
create policy "profiles_select" on profiles
  for select using (auth.uid() = id or is_admin() or is_instructor());
...
create policy "profile_stats_select" on profile_stats
  for select using (auth.uid() = user_id or is_admin() or is_instructor());
...
create policy "quizzes_insert" on quizzes
  for insert with check (is_instructor() and instructor_id = auth.uid());
...
create policy "quiz_stats_select" on quiz_stats
  for select using (
    exists (
      select 1 from quizzes q
      where q.id = quiz_id
        and (q.status = 'published' or q.instructor_id = auth.uid() or is_admin())
    )
  );
...
create policy "quiz_versions_select" on quiz_versions
  for select using (
    exists (
      select 1 from quizzes q
      where q.id = quiz_id and (q.instructor_id = auth.uid() or is_admin())
    )
  );

create policy "quiz_versions_insert" on quiz_versions
  for insert with check (is_instructor());
...
create policy "questions_select" on questions
  for select using (
    quiz_id is null  -- question bank: all authenticated
    or exists (
      select 1 from quizzes q
      where q.id = quiz_id
        and (q.status = 'published' or q.instructor_id = auth.uid() or is_admin())
...
create policy "attempts_select_own" on quiz_attempts
  for select using (user_id = auth.uid() or is_admin() or is_instructor());
...
create policy "answers_select" on attempt_answers
  for select using (
    exists (
      select 1 from quiz_attempts a
      where a.id = attempt_id and (a.user_id = auth.uid() or is_admin() or is_instructor())
    )
  );

create policy "answers_insert" on attempt_answers
...
create policy "notifications_insert" on notifications
  for insert with check (is_admin() or is_instructor() or user_id = auth.uid());
