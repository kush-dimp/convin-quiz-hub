...
const SortBtn = ({ field, active, dir, onClick }) => (
  <button onClick={onClick} className="inline-flex items-center gap-0.5 hover:text-slate-900 transition-colors">
    ...
    {active
      ? (dir === 'asc'
          ? <ChevronUp   className="w-3 h-3 text-[#FF6B9D]" />
          : <ChevronDown className="w-3 h-3 text-[#FF6B9D]" />)
  ...
  if (resultsLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-8 h-8 border-[3px] border-[#FFB3C6] border-t-[#E63E6D] rounded-full animate-spin" />
      </div>
    )
  }
        ...
        <div className="glass-card rounded-2xl px-4 py-3 flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-44">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[13px] text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#FFB3C6] focus:border-[#FF6B9D]/60 transition-all"
            />
          </div>
          <select value={filterPass} onChange={e => setFilterPass(e.target.value)}
            className="border border-slate-200 rounded-xl px-3 py-2 text-[13px] bg-white text-slate-600 focus:outline-none focus:ring-2 focus:ring-[#FFB3C6]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="w-10 px-4 py-3.5">
                    <input type="checkbox" className="rounded accent-[#E63E6D]"
              ...
              <tbody>
                {paged.map(r => (
                  <tr key={r.id}
                    className={`border-b border-slate-50 hover:bg-slate-50/70 transition-colors ${selected.has(r.id) ? 'bg-[#FFF5F7]/40' : ''}`}>
                    <td className="px-4 py-3.5">
                      <input type="checkbox" checked={selected.has(r.id)} onChange={() => toggleSelect(r.id)} className="rounded accent-[#E63E6D]" />
                    </td>
                    <td className="px-4 py-3.5">
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-7 h-7 rounded-lg text-[12px] font-medium transition-colors ${
                    page === p ? 'bg-[#E63E6D] text-white shadow-sm' : 'border border-slate-200 text-slate-500 hover:bg-slate-50'
                  }`}>
                  {p}
                </button>
              ))}
